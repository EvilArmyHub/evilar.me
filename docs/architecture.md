...
Поиск работает поверх Pagefind. Текущая реализация использует `@pagefind/default-ui` внутри собственного модального окна; план миграции на Pagefind Component UI лежит в `PAGEFIND_COMPONENT_UI_MIGRATION.md`.
...
- миграцию на Component UI нужно делать отдельной задачей, потому что меняются dependency, HTML web components, CSS selectors и поведение фокуса/горячих клавиш

### Theme

Тема работает через `data-theme` на `document.documentElement`.

- `src/components/ThemeProvider.astro` — ранняя инициализация темы, синхронизация с system preference, Astro view transitions и восстановлением страницы из back/forward cache
...
- `biome.json` включает Tailwind directive parsing для CSS 4 directives
...
- `src/pages/og-image/[...slug].png.ts` — endpoint генерации OG изображений
- `src/pages/og-image/_markup.ts` — HTML/Tailwind разметка для Satori
- `src/pages/og-image/_cache.ts` — кеш PNG в Astro cache dir, ключованный версией кеша, заголовком и датой поста
- Satori генерирует SVG, Sharp конвертирует SVG в PNG
- `src/styles/global.css` исключает `src/pages/og-image` из Tailwind source scanning через `@source not "../pages/og-image"`
...
- Pinned posts на главной выбираются из полного отсортированного списка постов, а не только из последних N публикаций.
